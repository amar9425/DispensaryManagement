...
const MedicineModal = (props)=>{
    ...
    const updateValue = async()=>{
        props.showLoader();
        await axios.put(`${API}/api/medicine/update/${props.clickedMedicine._id}`,medicine,{withCredentials:true}).then((response)=>{
    ...
    const handleSubmit= async(event)=>{
       ...
       props.showLoader();
       await axios.post(`${API}/api/medicine/add`,medicine,{withCredentials:true}).then((response)=>{
        window.location.reload();
       }).catch(err=>{
        toast.error(err?.response?.data?.error);
       }).finally(()=>{
